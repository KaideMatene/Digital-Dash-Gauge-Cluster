...
class GaugePreview(QMainWindow):
    ...
    def __init__(self):
        ...
        tach_scale = tach_cal.get('needle_scale', TACH_BASE_SCALE) if tach_cal else TACH_BASE_SCALE
        ...
        speed_scale = speed_cal.get('needle_scale', SPEED_BASE_SCALE) if speed_cal else SPEED_BASE_SCALE
        ...
        fuel_scale = fuel_cal.get('needle_scale', FUEL_BASE_SCALE) if fuel_cal else FUEL_BASE_SCALE
        ...
        self.demo_boost_build_ratio = 0.35
        self.demo_boost_initial_progress = 0.15
        self.demo_accel_windows = [
            {"start": 7.0, "end": 9.0, "rpm_start": 2000, "rpm_end": 7000, "speed_start": 0, "speed_end": 65},
            {"start": 10.5, "end": 13.5, "rpm_start": 2000, "rpm_end": 7500, "speed_start": 62, "speed_end": 105},
            {"start": 15.0, "end": 18.0, "rpm_start": 2500, "rpm_end": 7500, "speed_start": 102, "speed_end": 145},
            {"start": 19.5, "end": 22.5, "rpm_start": 3000, "rpm_end": 7500, "speed_start": 141, "speed_end": 193},
            {"start": 24.0, "end": 27.0, "rpm_start": 3500, "rpm_end": 8000, "speed_start": 189, "speed_end": 255},
    ...
    def _create_demo_keyframes(self):
        ...
        keyframes = [
            ...
            # 1st Gear (7-9s): 2000→7000 rpm in 2s, 0→65 km/h, water 50→75
            (7.0, 2000, 0, 100, 50),
            (9.0, 7000, 65, 98, 75),
            ...
            # 2nd Gear (10.5-13.5s): 2000→7500 rpm in 3s, 62→105 km/h, water 75→85
            (10.5, 2000, 62, 97, 75),
            (13.5, 7500, 105, 95, 85),
            ...
            # 3rd Gear (15-18s): 2500→7500 rpm in 3s, 102→145 km/h, water 85→95
            (15.0, 2500, 102, 94, 85),
            (18.0, 7500, 145, 92, 95),
            ...
            # 4th Gear (19.5-22.5s): 3000→7500 rpm in 3s, 141→193 km/h, water 95→100
            (19.5, 3000, 141, 91, 95),
            (22.5, 7500, 193, 89, 100),
            
            # Rev downfall (22.5-24s): 7500→3500 rpm over 1.5s
            (24.0, 3500, 189, 88, 100),
            
            # 5th Gear (24-27s): 3500→8000 rpm in 3s, 189→255 km/h, water 100→110
            (24.0, 3500, 189, 88, 100),
            (27.0, 8000, 255, 86, 110),
            
            # Rev downfall (27-28.5s): 8000→4000 rpm over 1.5s
            (28.5, 4000, 250, 85, 110),
            
            # 6th Gear (28.5-34.5s): 4000→10000 rpm in 6s, 250→320 km/h, water 110→130
            (28.5, 4000, 250, 85, 110),
            (34.5, 10000, 320, 83, 130),
            
            # Deceleration (34.5-64.5s): 30 seconds to come to a stop
            (34.5, 10000, 320, 83, 130),
            (37.0, 6000, 310, 82, 130),    # 5th gear downshift
            (38.5, 2500, 300, 81, 130),    # Coast
            (40.0, 5000, 290, 80, 130),    # 4th gear rev
            (41.5, 2000, 270, 79, 130),    # Coast
            (43.0, 4000, 250, 78, 130),    # 3rd gear rev
            (44.5, 1500, 220, 77, 130),    # Coast
            (46.0, 3000, 180, 76, 130),    # 2nd gear rev
            (47.5, 1000, 120, 74, 130),    # Coast
            (49.0, 2000, 60, 72, 130),     # 1st gear
            (64.5, 900, 0, 70, 130),       # Back to idle
            (65.0, 0, 0, 0, 130),          # Stop
        ]
        return keyframes
    ...
    def _apply_boost_curve(self, time, rpm, speed):
        for window in self.demo_accel_windows:
            if window["start"] <= time <= window["end"]:
                duration = window["end"] - window["start"]
                if duration <= 0:
                    return rpm, speed

                local_t = time - window["start"]
                build_time = duration * self.demo_boost_build_ratio
                if local_t <= build_time:
                    progress = (local_t / build_time) * self.demo_boost_initial_progress
                else:
                    remaining_t = local_t - build_time
                    remaining_duration = duration - build_time
                    progress = self.demo_boost_initial_progress + (remaining_t / remaining_duration) * (1.0 - self.demo_boost_initial_progress)

                rpm = window["rpm_start"] + (window["rpm_end"] - window["rpm_start"]) * progress
                speed = window["speed_start"] + (window["speed_end"] - window["speed_start"]) * progress
                return rpm, speed
        return rpm, speed
    ...
    def _update_demo(self):
        ...
        rpm, speed = self._apply_boost_curve(self.demo_elapsed_time, rpm, speed)
        ...
        # Update water gauge
        self.fuel.set_named_needle_value("water", water)
