...
class ImageBasedGauge(QWidget):
    ...
    def __init__(self, bg_image_path: str, needle_image_path: str = None, parent=None):
        ...
        self.base_needle_scale = 1.0  # Fixed base scale so 1.0x UI matches correct size
        ...
        # Symbol support: list of symbols to render on this gauge
        self.symbols = []  # [{'image_path': str, 'position_x': int, 'position_y': int, 'scale': float, 'visible': bool}]
    ...
    def set_symbols(self, symbols: list):
        """
        Set symbols to render on this gauge.

        Args:
            symbols: List of dicts with keys: 'image_path', 'position_x', 'position_y', 'scale', 'visible'
        """
    ...
    def paintEvent(self, event):
        ...
        if not self.bg_pixmap.isNull():
            ...
            # Draw symbols AFTER background but BEFORE needle
            self._draw_symbols(painter, gauge_x, gauge_y, gauge_width, gauge_height)
        else:
    ...
    def _draw_symbols(self, painter: QPainter, gauge_x: float, gauge_y: float, gauge_width: float, gauge_height: float):
        """Draw symbols on the gauge at their configured positions"""
        if not self.symbols:
            return

        for symbol in self.symbols:
            # Only draw visible symbols
            if not symbol.get('visible', False):
                continue

            image_path = symbol.get('image_path')
            if not image_path:
                continue

            # Load symbol image
            symbol_pixmap = QPixmap(image_path)
            if symbol_pixmap.isNull():
                logger.warning(f"⚠️ Failed to load symbol: {image_path}")
                continue

            # Get position in original gauge coordinates
            pos_x = symbol.get('position_x', 0)
            pos_y = symbol.get('position_y', 0)
            scale = symbol.get('scale', 1.0)

            # Debug: Log symbol rendering
            logger.debug(f"🎯 Rendering symbol '{symbol.get('id', 'unknown')}' at original coords ({pos_x}, {pos_y})")

            # Convert from original gauge coordinates to display coordinates
            if self.gauge_bg_width and self.gauge_bg_height:
                # Scale position from original gauge size to display size
                display_x = gauge_x + (pos_x / self.gauge_bg_width) * gauge_width
                display_y = gauge_y + (pos_y / self.gauge_bg_height) * gauge_height

                logger.debug(f"   → Display coords: ({int(display_x)}, {int(display_y)}) | Gauge BG: {self.gauge_bg_width}x{self.gauge_bg_height} | Display: {int(gauge_width)}x{int(gauge_height)}")

                # Scale symbol size
                symbol_scale = (gauge_width / self.gauge_bg_width) * scale
                scaled_width = int(symbol_pixmap.width() * symbol_scale)
                scaled_height = int(symbol_pixmap.height() * symbol_scale)

                if scaled_width > 0 and scaled_height > 0:
                    scaled_symbol = symbol_pixmap.scaled(
                        scaled_width,
                        scaled_height,
                        Qt.KeepAspectRatio,
                        Qt.SmoothTransformation
                    ...
                    # Center the symbol at the position
                    symbol_x = display_x - scaled_symbol.width() / 2
                    symbol_y = display_y - scaled_symbol.height() / 2

                    painter.drawPixmap(int(symbol_x), int(symbol_y), scaled_symbol)
                    logger.debug(f"   ✅ Symbol rendered at display position ({int(symbol_x)}, {int(symbol_y)})")
    ...
    def _rotate_and_draw_needle(self, painter: QPainter, center: QPointF, angle: float, radius: float):
        ...
        if self.needle_length_px and desired_length:
            desired_length *= (self.needle_scale * self.base_needle_scale)
            ...
            needle_height = int(radius * 1.5 * self.needle_scale * self.base_needle_scale)
    ...
    def _draw_svg_needle(self, painter: QPainter, center: QPointF, radius: float, angle: float):
        ...
        if self.desired_tip_radius_px and self.gauge_display_scale and self.gauge_bg_width and self.gauge_bg_height and self.desired_tip_radius_px > 0:
            ...
            desired_length = radius * needle_percentage_of_radius * self.needle_scale * self.base_needle_scale
...
class ImageFuelGauge(ImageBasedGauge):
    ...
    def __init__(self, bg_image_path: str, needle_image_path: str = "gauges/needle.png", parent=None):
        ...
        self.water_base_needle_scale = 1.0  # Fixed base scale for water needle
    ...
    def paintEvent(self, event):
        ...
        # Draw symbols AFTER background but BEFORE needles
        self._draw_symbols(painter, gauge_x, gauge_y, gauge_width, gauge_height)
    ...
    def _draw_water_needle(self, painter: QPainter, center: QPointF, radius: float):
        ...
        if self.water_needle_length_px and desired_length:
            desired_length *= (self.water_needle_scale * self.water_base_needle_scale)
            ...
            needle_height = int(radius * 1.5 * self.water_needle_scale * self.water_base_needle_scale)
